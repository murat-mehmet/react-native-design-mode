...
export class DesignMode extends Component<DesignModeProps> {
    state = {
        ...
        shown: true,
        pages: [] as {title: string; prepare: () => Promise<any>; Component: ReactNode}[],
        showPage: false,
        currentPage: 0,
    ...
    load = () => {
        ...
        React.Children.forEach(children, (element) => {
            ...
            if (element.type == DesignPage) {
                pages.push({title: element.props.title, prepare: element.props.prepare, Component: element});
            }
        });
                ...
                <KeyboardAvoidingView style={styles.showButtonContainer} behavior="position" enabled={Platform.OS == 'ios'}>
                    <TouchableOpacity
                        ...
                        onPress={() => {
                            if (!this.state.shown) {
                                this.load();
                            } else if (this.state.showPage && !this.state.showPageSelector) {
    ...
    renderPageSelector = () => {
        return (
            <View style={{flex: 1, backgroundColor: 'white'}}>
                <Text style={{color: 'black', textAlign: 'center', marginTop: 15, fontWeight: 'bold'}}>Select a design</Text>
                <ScrollView contentContainerStyle={{paddingBottom: 15}}>
                    {this.state.pages.map((x, i) => (
                        <TouchableOpacity
                            key={i}
                            style={{marginHorizontal: 15, marginTop: 15, padding: 15, borderWidth: 1, borderRadius: 10}}
                            onPress={() => {
                                const page = this.state.pages[i];
                                this.setState({currentPage: i, showPageSelector: false, ready: !page.prepare});
                                if (page.prepare)
                                    page.prepare()
                                        .then(() => this.setState({ready: true}))
                                        .catch((e) => this.setState({prepareError: e}));
